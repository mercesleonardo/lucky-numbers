...
class ImportLotteryHistorical extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'lottery:import-historical
                            ...
                            {--from= : Número do concurso inicial (padrão: 1)}
                            {--to= : Número do concurso final (padrão: último disponível)}
     ...
     * @var string
     */
    protected $description = 'Importa TODOS os concursos históricos dos jogos de loteria da API das Loterias da Caixa';
    ...
    public function handle(LotteryGameService $lotteryService): int
    {
        ...
        $this->info('📚 Iniciando importação histórica de dados de loteria...');

        try {
            if ($this->option('all')) {
                return $this->importAllGamesHistorical($lotteryService);
            }
            ...
            return $this->importSingleGameHistorical($lotteryService, $game);
    ...
    /**
     * Importa histórico de todos os jogos
     */
    private function importAllGamesHistorical(LotteryGameService $lotteryService): int
    {
        ...
        // Relatório final
        $this->newLine(2);
        $this->info('📊 RELATÓRIO FINAL DA IMPORTAÇÃO HISTÓRICA:');
        $this->table(
            ['Métrica', 'Valor'],
            [
    ...
    /**
     * Importa histórico de um jogo específico
     */
    private function importSingleGameHistorical(LotteryGameService $lotteryService, string $game): int
    {
        ...
        if ($from || $to) {
            ...
            $this->info("📈 Importando {$rangeText} de: {$game}");
        } else {
            $this->info("📈 Importando histórico completo de: {$game}");
        }
